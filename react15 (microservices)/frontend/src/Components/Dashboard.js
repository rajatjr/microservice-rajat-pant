...
function Product() {
  ...
  const handleSubmit = (event) => {
    ...
    console.log(user_id)
    axios.post('http://localhost:3500/api/addProducts', { productName, productPrice, category, userId: user_id })
      .then(res => {
        console.log(res);
  ...
  const fetchInfo = async () => {
    try {
      const response = await axios.get('http://localhost:3500/api/getProducts');
  ...
  useEffect(() => {
    const searchProduct = async () => {
      try {
        const res = await axios.get(`http://localhost:3500/api/searchapi?productName=${searchInput.toLowerCase()}&user_id=${user_id}`);
  ...
  const Delete = async (id) => {
    ...
    try {
      ...
      if (confirmed) {
        const res = await axios.delete(`http://localhost:3500/api/delete/${id}`, {
          headers: {
  ...
  const handleAddTOcart = (id) => {
    ...
    axios.post('http://localhost:3500/api/AddtoCart', { id, userId: user_id, ProductCount: ProductCount })
      .then(res => {
