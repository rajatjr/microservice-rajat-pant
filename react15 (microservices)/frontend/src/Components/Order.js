...
function Order() {
  ...
  const fetchOrder = async () => {
    try {
      const response = await axios.get(`http://localhost:3500/api/getProductsfromOrder?user_id=${user_id}`);
  ...
  const Delete = async (id) => {
    ...
    try {
      ...
      if (confirmed) {
        const res = await axios.delete(`http://localhost:3500/api/DeleteOrder?id=${id}`, {
          headers: {
